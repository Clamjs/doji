...
mace.Klass(Doji, {
  config: function (options) {
    ...
    if (widgets && widgets.selectors && widgets.rootdir) {
      this.widgets = Doji.Widgets(widgets);
    }
    return this;
  },
  ...
  proxy: function (req, res) {
    ...
    var preq = (req.isSecurity ? https : http).request(req.options, function (pres) {
      ...
      pres.on('end', function () {
        ...
        // 合并
        buff = Doji.joinbuffer(buff);
        if (contentType.indexOf('html') === -1) {
          debug('Not a html send directly!');
          return self.send(req, res, pres, buff)
        }
        // 去BOM
        buff = Doji.nobom(buff);
        // 解压
        buff = Doji.zlib(buff, pres);
        // 解码
        buff = Doji.charset(buff, pres);
        // 判断是否html页面
        if (buff.toString('utf-8').match(/^\s*\<\!doctype\s/i)) {
          ...
          if (self.widgets) {
            ...
            buff = self.widgets.replace(buff, pres);
          }
          // 更新时间戳
          if (conf.timestamp) {
            debug('Update the source timestamp now;')
            buff = Doji.timestamp(buff, pres);
          }
          mace.each(conf.parsers, function (parse) {
            if (parse && (parse = parse(buff, pres))) {
              buff = parse;
              parse = null;
            }
          });
